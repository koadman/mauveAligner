...
mutating to $Y$.
Because the background frequencies on which the substitution matrix was computed are
...
\end{center}
where $f'_{AT}$ and $f'_{GC}$ represent the input genome's AT and GC content, $H_{XY}$ 
values are the original emission probabilities derived from the
substitution matrix, and $H'_{XY}$ are emission probabilities adapted to 
the input genome's G+C content.  \textbf{TODO: should we put a more detailed derivation in the appendix?}
...
Whereas emission frequencies for nucleotide substitutions can be derived from
