...
int main( int argc, char* argv[] ){
	...
	int rval = doAlignment(argc, argv);
...
try{
	...
	if( seq_files.size() > MAX_SEQ_COUNT ){
		cerr << "This copy of mauveAligner was compiled to align a maximum of " << MAX_SEQ_COUNT << " sequences\n";
		cout << "This copy of mauveAligner was compiled to align a maximum of " << MAX_SEQ_COUNT << " sequences\n";
		cout.flush();
		return -1;
	}
	...
	if( seq_files.size() == 1 && sml_files.size() == 0 ){
		...
		if( match_list.seq_table.size() > MAX_SEQ_COUNT ){
			cerr << "This copy of mauveAligner was compiled to align a maximum of " << MAX_SEQ_COUNT << " sequences\n";
			cout << "This copy of mauveAligner was compiled to align a maximum of " << MAX_SEQ_COUNT << " sequences\n";
			cout.flush();
			return -1;
	...
	if( read_matches ){
		...
		if( !lcb_match_input_format )
		{
			...
			for( int ivI = 0; ivI < m_iv_list.size(); ivI++ ){
				for( int mI = 0; mI < m_iv_list[ivI].matches.size(); mI++ ){
					Match* m = dynamic_cast< Match* >(m_iv_list[ivI].matches[mI]);
					if( m != NULL )
	...
	if( !create_LCBs && !read_lcbs){
		...
		if( eliminate_overlaps ){
			MpiAligner::EliminateOverlaps( match_list );
		}
		...
		if( tree_filename != "" ){
			...
			if( tree_filename == "" ){
				wxString temp_treename = wxGetTempFileName("guide_tree");
				tree_filename = temp_treename.c_str();
			}
...
//			addUnalignedIntervals( interval_list );
	...
	if( realign_lcbs.size() > 0 ){
		...
		for( int realignI = 0; realignI < realign_lcbs.size(); realignI++ ){
			...
			match_list.clear();
			for( int matchI = 0; matchI < iv.matches.size(); matchI++ ){
				AbstractMatch* m = iv.matches[ matchI ];
	...
	if( alignment_output_dir != "" ){
		...
		wxString wxalignment_output_dir( alignment_output_dir.c_str() );
		if( !wxDirExists( wxalignment_output_dir ) ){
			wxMkdir( wxalignment_output_dir, 755 );
		}
		wxSetWorkingDirectory( wxalignment_output_dir );
		for( lcbI = 0; lcbI < interval_list.size(); lcbI++ ){
			...
			ostringstream oss;
			oss << "lcb_" << lcbI << ".txt";
			ofstream alignment_lcb_out( oss.str().c_str(), ios::trunc );
...
void print_usage( const char* pname ){
	...
	cerr << "\t    --lcb-input=<file> Use specified lcb file instead of constructing LCBs\n";
