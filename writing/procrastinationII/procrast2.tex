...
\url{http://somewhere.net}\textbf{FIXME: find a permanent home for
...
would provide an increasingly accurate alignment.  Highly repetitive
...
sequence, we utilize a palindromic spaced seed pattern, which is
...
potentially more sensitive.  We have experimentally determined that
...
substitution matrix presented by Chiaramonte \textit{et al}
2002\cite{hoxd}. We solved for the emission frequencies in the
...
where $p(x,y)$ is the fraction of the observed aligned pairs of
...
any strand/species symmetric nucleotide substitution matrix, but gap
open and extend frequencies can not.  To empirically estimate values
for the unrelated state we aligned a 10kb, 48\% GC content region
...
alignment with MUSCLE and counted the number of gap open and gap
extend columns in the alignment of unrelated sequences.  Gap open and
...
the figure, it is apparent that \texttt{procrastAligner} performs well
...
parallel using a 128 node compute cluster.  Results of the experiments
...
all 12 sequences. Start positions are shown to the left, negative
