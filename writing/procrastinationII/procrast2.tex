...
homologous and unrelated sequence. The input genome for this example has $47.5\%$~$G+C$ content. Emission probabilities correspond to alignment
...
Bacterial genomes range in GC content from 30-75\%, and given that the training
data has $47.5\%$~$G+C$ content,the extremes our HMM is not likely to perform very well.  We instead
...
\begin{equation}
\label{eq2}
\log_{2}\bigg(\frac{H_{AA}}{U_{AA}}\bigg) = \frac{91}{\psi}\  \ \Rightarrow \ \ \  H_{AA} = 2^{(\frac{91}{\psi})}U_{AA}
\end{equation}
where $\psi$ is scaling factor used normalize $H_{CC}$ to 100. The full list of equations follows:
\begin{center}
$H_{AC} = 2^{(\frac{-114}{\psi})}U_{AC},$\ \ \
$H_{AG} = 2^{(\frac{-31}{\psi})}U_{AC},$\ \ 
$H_{AT} = 2^{(\frac{-123}{\psi})}U_{AC}$ \\
$H_{CG} = 2^{(\frac{-123}{\psi})}U_{CC},$\ \ \ \  
$H_{AA} = 2^{(\frac{91}{\psi})}U_{AA},$\ \ 
$H_{CC} = 2^{(\frac{100}{\psi})}U_{CC}$ \\
\end{center}
...
%$0.03072937146$
%$32.54215601$
We can solve the above equation for $H_{xy}$ by substituting the
resulting expressions from Equation~\ref{eq2} in to Equation~\ref{eq3}. 
As a result, we can calculate values for all $H_{xy}$ in the HMM using the
background nucleotide frequencies of the input genome.
\linebreak
