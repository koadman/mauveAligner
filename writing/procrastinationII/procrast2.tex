...
$\frac{TP}{TP + FP}$, where TP is defined as above, and FP is the total number of nucleotide pairs from the program's output where one or both nucleotides are part of the simulated repeat family and the pair of nucleotides was incorrectly aligned.  \textbf{fixme: how was it done?  the way i just described, or the way you've described here: with
one of the tuple components are present in the simulated data yet it doesn't exist in the simulated data??}.  

\subsection{Accuracy when aligning interspersed repeats}
We applied \textt{procrastAligner} and \texttt{eulerAlign} to the hybrid simulated/real dataset.  Parameters used for each program were configured to optimize each program's chance of accurately aligning each pattern as follows:  \begin{center}\texttt{procrastAlign}:\texttt{ --z=13 --w=50 } \\ \texttt{eulerAlign}:\texttt{ -k 15 -l -i 1000 -v}\end{center}
