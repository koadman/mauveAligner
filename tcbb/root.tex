...
\begin{figure}[t!]
\begin{center}
\epsfig{file=./fig/extension.pdf,width=3.5in}
...
\begin{figure*}[t!]
\centering \epsfig{file=./fig/hmm.pdf,width=5.6in}
...
\begin{figure*}[t!]
\centering \epsfig{file=./fig/repeat_accuracy_new.pdf,width=5.8in}
...
\begin{figure*}[t!]
\centering
\epsfig{file=./fig/boundary_scores.pdf,width=5.8in}
...
\centering
\subfigure[]{
\epsfig{file=./fig/procrast_boundary_fig.pdf,width=3.2in}
\label{fig:subfig1}}
\subfigure[]{
\epsfig{file=./fig/euler_boundary_fig.pdf,width=3.2in}
\label{fig:subfig2}}
...
insignificant local alignments is essential.  BLAST statistics have proven essential for searching for significant similarity hits, for amino acid and nucleotide sequences. Exact computation of $p$-values for local multiple alignments remains a
...
extended to multiple alignments\cite{ref-related1, Prakash2005}.  We will
now review the method we use for estimating significance of local multiple alignments.

Differing from similarity between two or more sequences, there exists random similarities within a sequence. Therefore, one is interested in computing the likelihood that a given repeat is found by chance in order to decide whether it is biologically pertinent or not. A lot of effort has been put to assign a probability of finding a segment of score $S$ between two sequences (see \cite{Ewens2001} and references therein) or shared by $k$ sequences \cite{ref-related1}. These statistics show that, $N$, the number of high scoring similarity segments between two sequences of size $m$ and $n$ with a score $S \geq x$ converges to \cite{Karlin1990}:
...
$K$ is a scale parameter that depends on the scoring system and $p$ (also often seen as $p=e^{-\lambda}$) can be understand as the per site probability to find a segment of score 1. Using a Poisson approximation, the probability that the largest score is greater or equal than $x$ is given by:
...
These results can be easily extended to 2-copy repeats within the same strand of a sequence of size $n$, by substituting $mn$ by ${{n}\choose{2}}$ in the above equations. When the two copies can belongs to either strand, we have to count successively cases where 0,1 or 2 copies are on the forward strand; copies of the same strand can swapped without changing the repeat. Because of the symmetry, the number of possible positions is halved. Therefore, the expected number of 2-copy repeats with a score greater or equal than $x$ is:
...
More generally, the expected number of repeats of multiplicity $r$ on both strands that have a score greater or equal than $x$ is given by:
...
In the above equation, the two unknown parameters $K$ and $p$ can be estimated on the 2-copy repeats using random sequences of the same size and identical composition. This can be done by using either the direct method \cite{Olsen1999} using a $log(-log(1-P))$ transformation of the equation \ref{P_s1} or by using the declumping \cite{Waterman1994} or the island method \cite{Olsen1999} using a $log(E)$ of equation \ref{E_r2}. Once $K$ and $p$ are estimated, equation \ref{P_s1} along with equation \ref{E_repeat} can be used to asses a probability of any observed repeat of a given score and a given multiplicity. The adequate score to be used is the average score of all pairs.  Importantly, these equations show a perfect fit with repeats from random genomes (data not shown).

The introduction of a preliminary method for estimating significance of local multiple alignments will allow
for increased sensitivity (i.e. smaller seed size, seed families) while maintaining specificity. This statistical
...
at the alignment in the group with the highest Sum-of-Pairs (SP) score, breaking ties either by multiplicity or repeat length. The selection of a reference alignment from each redundant group can be defined by the user via a command-line parameter, allowing for the reference alignment to be  on the type of repeat analysis performed. 
...
\begin{figure}[t!]
\centering \epsfig{file=./fig/alu_align.pdf,width=3.5in}
